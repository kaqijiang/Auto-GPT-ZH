...
def llm_call(
    ...
    cn_prompt = "Please always respond in Chinese.\nIf you encounter technical terms, you can add the English original text in parentheses after the Chinese."
    system_messages = [p for p in prompt if p.get("role") == "system"]
    
    if not system_messages:
        # No system messages, add cn_prompt requirement
        prompt.insert(0, {"role": "system", "content": cn_prompt})
    else:
        # Has system messages, check if Chinese requirement exists
        has_chinese = any(cn_prompt in msg.get("content", "") for msg in system_messages)
        if not has_chinese:
            # Add Chinese requirement to first system message
            system_messages[0]["content"] = cn_prompt + "\n\n" + system_messages[0]["content"]
